...
class Controller extends BlockController
{
    ...
    protected $iconStyles = '
		.ccm-block-fa-social-media-icons .icon-container .svc { height: %iconSize%px; width: calc(%iconSize%px + %iconMargin%px); }
		.ccm-block-fa-social-media-icons .icon-container .svc.activated span { %activeAttrs% }
		.social-icon:hover { %hoverAttrs% }
		.social-icon.activated, .social-icon.activated:hover { %activeAttrs% }
...
<style id="iconStyles" type="text/css">
    ...
    private function genIcons()
    {
		...
		foreach ($mediaListMaster as $key => $mProps)
    	{
    		...
    		$icon =  '<span class="' . $iconClass . '"><i class="fa fa-' . $mProps['fa'] . '" title="' . t('Visit our page at %s.', $key). '"></i></span>';
    		...
    		if ($props['checked'])
    		{
    			...
    			if ($key == 'Skype' || $key == 'Email')
    				$trg = '_self';
				$html = '
					<div class="svc '. $mProps['fa'] . '" data-href="' . $props['url'] . '" data-target="' . $trg . '">
					   ' . $icon . '
					   <div class="speech-bubble">
						   <input type="checkbox" checked="checked" id="'. $mProps['fa'] . '-'. $this->bID . '">
						   <label for="'. $mProps['fa'] . '-'. $this->bID . '">' .
							   t('You now have enabled the button to visit out page at &quot;%s&quot;. ' .
								 ...
								 'To avoid this you can disblae the checkbox at left (and the enabled button).', $key, $key, $key) . '
					   </div>
				   </div>';
    			$this->mediaList[$key]['html'] = $html;
    		}
